...
def get_product_data(brands, category_id, page):
    ...
    for product in data["list"]:
        price = product["price"]
        if not any(brand in product["name"] for brand in brands) or not price.isdigit() or price[-1] != "0":
            continue
        ...
        product_info = {
            ...
            "update_times": [product["update_time"]]
        }
    ...
    for new_product in new_products:
        for product in all_products:
            if product["pid"] == new_product["pid"]:
                ...
                if new_product["prices"][0] not in product["prices"]:
                    product["prices"].append(new_product["prices"][0])
                if new_product["update_times"][0] not in product["update_times"]:
                    product["update_times"].append(new_product["update_times"][0])
                break
        else:
...
if __name__ == "__main__":
    brand_names_file = "brands.json"
    output_file = "bunjang_products_satur.json"

    brand_names = load_brand_names(brand_names_file)
    collect_data(brand_names, output_file)