...
def extract_product_details(product_id):
    try:
        ...
        WebDriverWait(browser, 10).until(
            expected_conditions.presence_of_element_located((By.CSS_SELECTOR, "a[href*='/categories/']"))
        )
...
while not is_last_page:
    url = f"https://m.bunjang.co.kr/search/products?category_id={category_id}&order=date&page={page}&q={itemname}"
    browser.get(url)
    ...
    for item in product_list:
        ...
        if status==1:
            # 카테고리 넘버 뽑기
            category_number = extract_product_details(product_id)
