...
def main():
    ...
    for brand_name, alternative_brand_name in brands.items():
        ...
        filename = f"time/time_{brand_name}_data.json"
...
def bunjang_crawler(brand_name, alternative_brand_name, browser, category_id, crawl_end_page):
    ...
    while not is_last_page:
        retry_count = 0  # 페이지마다 재시도 카운터 초기화
        products_found = False
        while retry_count <= max_retries and not products_found:
            ...
            if product_list:
                ...
                if retry_count <= max_retries:
                    print(f"페이지 {page}에서 제품을 찾을 수 없습니다. {retry_count}/{max_retries} 재시도 중...")
                    time.sleep(2)  # 잠시 대기 후 재시도
                else:
                    print(f"페이지 {page}에서 제품을 찾을 수 없습니다. 재시도 후에도 제품이 없으므로 검색을 종료합니다.")
                    is_last_page = True  # 더 이상 페이지를 넘기지 않음
        if products_found:
